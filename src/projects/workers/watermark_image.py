...
class WatermarkImage(Worker):
    ...
    schema = {
        ...
        "properties": {
            ...
            "in_config": {
                "type": "object",
                "required": ["watermark_image", "gravity"],
                "properties": {
                    "watermark_image": {
                        "type": ["file", "string"],
            ...
            "out": {"type": "file", "description": "watermarked image"},
        },
    }
    ...
    def process(self, data):
        ...
        watermark_file = self.open_file(in_config["watermark_image"])
        ...
        if size:
            watermark = resizeimage.resize_thumbnail(
                watermark, [size.get("width"), size.get("height")]
            )

        img = Image.new("RGBA", (image.width, image.height), (0, 0, 0, 0))
        img.paste(image, (0, 0))
        position = (0, 0)
