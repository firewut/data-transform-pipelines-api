...
def process_pipeline(
    ...
    if not processors or len(processors) == 0 or error:
    ...
    for REGISTERED_WORKER_CLASS in REGISTERED_WORKER_CLASSES:
        if pipeline_processor.get("id") == REGISTERED_WORKER_CLASS.id:
            worker_instance = REGISTERED_WORKER_CLASS(
                pipeline_result=pipeline_result, pipeline_processor=pipeline_processor
            )
            ...
            try:
                result = worker_instance.execute(data)
            except jsonschema.exceptions.ValidationError as e:
                error = str(e)
                worker_instance.discard_files(data)
            except Exception as e:
                logger.error("[process_pipeline {}] Error: {}".format(result_id, e))
