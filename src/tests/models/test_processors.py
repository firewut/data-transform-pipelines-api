...
class ProcessorsTestCase(BaseTestCase):
    ...
    def test_check_in_config(self):
        self.processor.check_in_config(
            {"id": "test", "in_config": {"property": "extract_me"}}
        )
        ...
        with self.assertRaises(jsonschema.exceptions.ValidationError):
            self.processor.check_in_config({"id": "test"})

    def test_can_send_result(self):
        processor0 = Processor(
            schema={
                "type": "object",
                "properties": {"in": {"type": "integer"}, "out": {"type": "number"}},
            }
        )
        processor1 = Processor(
            schema={
                "type": "object",
                "properties": {"in": {"type": "number"}, "out": {"type": "boolean"}},
            }
        )

        # Correct Order
        self.assertTrue(processor0.can_send_result(processor1))

        # Incorrect Order
        self.assertFalse(processor1.can_send_result(processor0))
