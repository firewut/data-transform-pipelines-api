...
class PipelinesTestCase(PipelinesBaseTestCase):
    ...
    def test_create_pipeline(self, processors: [], status_code: int):
        ...
        if status_code == 400:
            self.assertEqual(
                response_json, {"processors": ["Processors does not exist: ['md666']"]}
            )
    ...
    def test_update_via_put(self):
        ...
        response, response_json = self.put_update(
            pipeline_data.get("id"), pipeline_data
        )
    ...
    def test_update_via_patch(self):
        ...
        _pipeline_data.update({"processors": processors})
        response, response_json = self.patch_update(
            _pipeline_data.get("id"), _pipeline_data
        )
        self.assertEqual(response.status_code, 200, response_json)
    ...
    def test_processors_correct_order(self):
        pipeline_data = {
            ...
            "processors": [
                {"id": "readability"},
                {"id": "get_object_property", "in_config": {"property": "content"}},
                {"id": "md5"},
            ],
        }
    ...
    def test_processors_incorrect_order(self):
        pipeline_data = {
            ...
            "title": self.random_string(),
            "project": self.project_id,
            "processors": [{"id": "readability"}, {"id": "md5"}],
        }
    ...
    def test_processors_incorrect_in_config(self):
        ...
        self.assertEqual(response.status_code, 400, response_json)
        self.assertIn(
            "random_string[0] has invalid in_config", str(response_json), response_json
        )
    ...
    def test_processors_valid_input_data(self, processor_id, in_config, in_data):
        pipeline_data = {
            ...
            "title": self.random_string(),
            "project": self.project_id,
            "processors": [{"id": processor_id, "in_config": in_config}],
        }
        response, response_json = self.put_create(pipeline_data)
        self.assertEqual(response.status_code, 201, response_json)

        response, response_json = self.put_update(
            pipeline_data.get("id"), in_data, action="process"
        )
        ...
        self.assertIn(
            "pipeline_result/{}/".format(response_json.get("id")),
            response["Location"],
        )
    ...
    def test_file_size_and_mimetype(self):
        pipeline_data = {
            ...
            "processors": [
                {
                    "id": "resize_image",
                    "in_config": {"size": {"width": 100, "height": 100}},
                },
            ],
        }
        response, response_json = self.put_create(pipeline_data)
        ...
        image_as_file = open(self.image_path, "rb")

        response, response_json = self.put_update(
            pipeline_data.get("id"),
            data={"file": image_as_file},
            action="process",
            _format="multipart",
        )
        self.assertEqual(response.status_code, 202, response_json)
...
class PipelinesProcessTestCase(PipelinesBaseTestCase):
    ...
    def setUp(self):
        ...
        pipeline_data = {
            ...
            "project": self.project_id,
            "processors": [
                {"id": "get_object_property", "in_config": {"property": "save_me"}}
            ],
        }
        response, response_json = self.put_create(
            pipeline_data, viewset=PipelineViewSet
        )
        self.assertEqual(response.status_code, 201, response_json)
    ...
    def test_pipeline_process_valid_data(self):
        ...
        response, response_json = self.get_item(pk=response_json.get("id"))
        self.assertEqual(response.status_code, 200, response_json)
        self.assertIsNone(response_json["error"])
        self.assertEqual(response_json["pipeline"], self.pipeline_id)
        self.assertTrue(response_json["is_finished"], response_json)
    ...
    def test_pipeline_process_valid_data_multiple_processors_json(self):
            ...
            "processors": [
                {
                    ...
                    "in_config": {"property": "markdown_here"},
                },
                {"id": "markdown"},
            ],
        }
        ...
        response, response_json = self.put_update(
            pipeline_id,
            {"data": {"markdown_here": "**Hello World**"}},
            action="process",
            viewset=PipelineViewSet,
        )
        self.assertEqual(response.status_code, 202, response_json)
    ...
    def test_pipeline_process_valid_data_multiple_processors_multipart(self):
        pipeline_data = {
            ...
            "processors": [
                {
                    "id": "resize_image",
                    "in_config": {"size": {"width": 200, "height": 200}},
                },
                {
                    "id": "watermark_image",
                    "in_config": {
    ...
    def test_pipeline_preview_json(self):
        ...
        self.assertTrue(response_json["is_finished"], response_json)
        self.assertEqual(
            response_json["result"], "202cb962ac59075b964b07152d234b70", response_json
        )
    ...
    def test_pipeline_web_hook_multipart(self, mocked_request):
        ...
        response, response_json = self.put_update(
            self.pipeline_id,
            {
                ...
                "processors": json.dumps(
                    [
                        {"id": "resize_image", "in_config": {"percentage": 15}},
                        {
                            "id": "web_hook",
                            "in_config": {
                                "method": method,
        ...
        self.assertEqual(call_args[0], (method, url))

        payload = call_args[1]
        self.assertIsNone(payload["data"])
        self.assertIn("files", payload)
        self.assertEqual(payload["headers"], {"Content-type": "multipart/form-data"})
    ...
    @mock.patch("requests.request")
    def test_pipeline_web_hook_multipart_consumed_by_file_processor(
        self, mocked_request
    ):
        response = ""
        mocked_request.return_value = self._fake_http_response(
