...
class HTMLToTextTestCase(WorkerBaseTestCase):
    ...
    @BaseTestCase.cases(
        ("<a>", "<a>hello foo</a> bar", "hello foo bar"),
        ("<div>", "<div>hello</div> foo <div>bar</div>", "hello foo bar"),
        ("unclosed tag", "<div>hello</div> foo <div>bar", "hello foo bar"),
        ("no_tags", "foo bar world", "foo bar world"),
    )
