...
class FetchURLTestCase(WorkerBaseTestCase):
    ...
    def test_worker(self, mocked_get):
        ...
        result = self.worker_class().execute("https://example.com")

        self.assertEqual(result, '"html content <hr />"')

    @mock.patch("requests.Session.options")
    def test_worker_no_methods(self, mocked_options):
        ...
        with self.assertRaises(WorkerNoInputException):
            self.worker_class().execute("https://example.com")

    @mock.patch("requests.Session.options")
    @mock.patch("requests.Session.head")
    def test_worker_content_too_big(self, mocked_head, mocked_options):
        mocked_options.return_value = self._fake_http_response(
            status=200, headers={"Allow": ["GET", "HEAD"]}
        )
        mocked_head.return_value = self._fake_http_response(
            status=200, headers={"Content-Length": 10000000 + 1}
        )
    ...
    @mock.patch("requests.Session.head")
    @mock.patch("requests.Session.get")
    def test_worker_meets_json(self, mocked_get, mocked_head, mocked_options):
        ...
        response = self.worker_class().execute("https://example.com")
    ...
    @mock.patch("requests.Session.head")
    @mock.patch("requests.Session.get")
    def test_worker_meets_csv(self, mocked_get, mocked_head, mocked_options):
        json_data = "a,b,c"
